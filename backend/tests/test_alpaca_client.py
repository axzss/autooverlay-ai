...
def test_network_error_is_exposed_as_safe_alpaca_error(monkeypatch):
    _configure_test_alpaca(monkeypatch)
    with patch("backend.app.alpaca_client.httpx.Client") as client_cls:
        client_cls.return_value.__enter__.return_value.request.side_effect = httpx.ConnectError("offline")
...
def test_invalid_json_is_exposed_as_safe_alpaca_error(monkeypatch):
    _configure_test_alpaca(monkeypatch)
    with patch("backend.app.alpaca_client.httpx.Client") as client_cls:
        client_cls.return_value.__enter__.return_value.request.return_value = _response()
