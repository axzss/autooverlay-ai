# AI-ENGINEER-REFERENCE — Module-by-Module

Companion to [`AI-ENGINEER.md`](AI-ENGINEER.md), which carries the layer summary
and the risk-architecture status table. This file is the detailed reference:
every module, its contract, and the reasoning behind each threshold.

Restored after `a2a2fcd` cut `AI-ENGINEER.md` from 358 to 98 lines. That commit
replaced working-code documentation with an architecture description; both are
useful, so the summary stayed there and the reference moved here rather than
being deleted.
...
4. **Consecutive stop-losses are within-cycle only** — `agent/state/peak.py`
   persists the high-water marks, but the `exit_event` ledger is not built, so
   three stops across three cycles still never halt. `KNOWN-ISSUES.md` #11.
5. **Handoff is unauthenticated** — see above.
6. **`monte_carlo.py` is not wired into anything** — a standalone stress tool
   whose default parameters halt 87.3% of paths, with no options in the model.
   See `AI-ENGINEER.md` §1C before quoting any figure from it.
